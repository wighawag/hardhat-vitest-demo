...
function setupBuidlerNetwork(env, api, ui) {
	...
	let networkName =
		env.buidlerArguments.network !== 'buidlerevm' ? env.buidlerArguments.network : api.defaultNetworkName;
...
async function setupHardhatNetwork(env, api, ui) {
	...
	if (networkName === HARDHAT_NETWORK_NAME) {
		...
		if (newCreateProviderSignature) {
			provider = await createProvider(env.config, networkName, env.artifacts);
		} else {
			provider = createProvider(networkName, networkConfig, env.config.paths, env.artifacts);
		}
...
async function finish(config, api, shouldKill) {
	const {tempContractsDir, tempArtifactsDir} = pluginUtils.getTempLocations(config);
