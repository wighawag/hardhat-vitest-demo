const UI = require('solidity-coverage/lib/ui').UI;
...
class PluginUI extends UI {
	constructor(log) {
		...
		this.flags = {
			...
			abi:
				`Generate a json object which can be used to produce a unified diff of your ` +
				`contracts public interface between two commits.`,

			solcoverjs:
				`Relative path from working directory to config. ` + `Useful for monorepo packages that share settings.`,

			temp:
				`Path to a disposable folder to store compilation artifacts in. ` +
	...
	report(kind, args = []) {
		...
		const kinds = {
			...
			'port-clash':
				`${w}  ${c.red("The 'port' values in your config's network url ")}` +
				`${c.red("and .solcover.js are different. Using network's: ")} ${c.bold(args[0])}.\n`,

			'port-clash-hardhat':
				`${w}  ${c.red("The 'port' values in your Hardhat network's url ")}` +
	...
	generate(kind, args = []) {
		...
		const kinds = {
			'network-fail':
				`${c.red('--network argument: ')}${args[0]}` + `${c.red(' is not a defined network in hardhat.config.js.')}`,

			'sources-fail': `${c.red('Cannot locate expected contract sources folder: ')} ${args[0]}`,

			'solcoverjs-fail':
				`${c.red('Could not load .solcover.js config file. ')}` +
